...
function FamilyPlanner({ family, families, onFamilyChange, isPlatformAdmin, canAdmin, adminOpen, setAdminOpen, displayName }: FamilyPlannerProps) {
  ...
  async function createTaskAndRefreshReminders(input: Parameters<typeof taskState.createTask>[0]) { await taskState.createTask(input); await reminderState.refresh() }
  async function updateTaskAndRefreshReminders(input: Parameters<typeof taskState.updateTask>[0]) { await taskState.updateTask(input); await reminderState.refresh() }
  ...
  return <div className="app-mobile-density min-h-screen bg-brand-bg text-brand-text">
    ...
    {quickTaskOpen ? <QuickTaskModal familyId={family.familyId} members={taskState.members} saving={taskState.saving} onCreate={createTaskAndRefreshReminders} onClose={() => setQuickTaskOpen(false)}/> : null}
    {taskToEdit ? <QuickTaskModal familyId={family.familyId} members={taskState.members} saving={taskState.saving} task={taskToEdit} reminder={reminderForSource(reminderState.reminders,'task',taskToEdit.id)} canManageRecurrence={canManageTaskAutomation(taskToEdit,family.userId,family.role)} onCreate={createTaskAndRefreshReminders} onUpdate={updateTaskAndRefreshReminders} onClose={()=>setTaskToEdit(null)}/> : null}
