...
export function MobileNav({ activeView, canBudget, canQuickAdd, onNavigate, onQuickAdd }: Props) {
  ...
  const item = (view: AppView, label: string, Icon: typeof Home) => <button type="button" onClick={() => go(view)} className={`grid h-full min-w-0 place-items-center content-center gap-0.5 text-[10px] leading-none ${activeView === view ? 'text-brand-gold' : 'text-brand-muted'}`}><Icon className="h-5 w-5"/><span className="max-w-full truncate px-0.5">{label}</span></button>
  ...
  return <>
    ...
    <nav className="mobile-nav fixed inset-x-0 bottom-0 z-50 border-t border-white/10 bg-[#0d0d13]/95 backdrop-blur lg:hidden" aria-label="Nawigacja mobilna"><div className="grid h-[60px] grid-cols-5 items-center px-1">{item('dashboard', 'Start', Home)}{item('calendar', 'Kalendarz', CalendarDays)}<div className="grid h-full place-items-center"><button type="button" disabled={!canQuickAdd} onClick={onQuickAdd} aria-label={quickAddLabel} className="grid h-12 w-12 place-items-center rounded-full bg-brand-gold text-black shadow-[0_5px_20px_rgba(255,216,77,.18)] disabled:opacity-40"><Plus className="h-5 w-5"/></button></div>{item('tasks', 'Zadania', CheckSquare)}<button type="button" onClick={() => setMore((value) => !value)} aria-expanded={more} className={`grid h-full min-w-0 place-items-center content-center gap-0.5 text-[10px] leading-none ${activeView === 'shopping' || activeView === 'budget' ? 'text-brand-gold' : 'text-brand-muted'}`}><MoreHorizontal className="h-5 w-5"/><span className="max-w-full truncate px-0.5">Więcej</span></button></div></nav>
  </>
}
