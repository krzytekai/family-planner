...
export function MobileNav({ activeView, canCreateTask, onNavigate, onQuickAdd }: MobileNavProps) {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 flex items-center justify-around border-t border-white/10 bg-[#0d0d13]/95 px-3 py-2 backdrop-blur lg:hidden">
      ...
      <button type="button" disabled title="Kalendarz — w przygotowaniu" className="grid cursor-not-allowed place-items-center gap-1 text-[10px] text-brand-muted/40"><CalendarDays className="h-5 w-5" />Kalendarz</button>
      <button type="button" disabled={!canCreateTask} onClick={onQuickAdd} aria-label="Dodaj szybkie zadanie" title={canCreateTask ? 'Dodaj szybkie zadanie' : 'Zadania mogą tworzyć dorośli i administratorzy'} className="grid h-12 w-12 place-items-center rounded-full bg-brand-gold text-black shadow-lg shadow-yellow-500/20 disabled:cursor-not-allowed disabled:opacity-40"><Plus className="h-6 w-6" /></button>
