...
export function AppHeader({ subtitle, displayName, unreadCount, onOpenNotifications, onLogout, onOpenAccount, family, families, onFamilyChange }: Props) {
  return <header className="app-topbar sticky top-0 z-30 flex h-20 items-center gap-2 border-b border-white/[.045] bg-brand-bg/88 px-4 backdrop-blur-xl md:px-7">
    ...
    <div className="app-topbar-actions ml-auto flex shrink-0 items-center gap-1 sm:gap-2">
      ...
      <button type="button" onClick={onOpenAccount} aria-label="Moje konto" title="Moje konto" className="app-avatar-shell flex items-center gap-2 rounded-xl border border-white/5 bg-white/[0.025] px-2.5 py-2"><span className="app-avatar grid h-8 w-8 place-items-center rounded-full bg-brand-gold/15 text-xs font-bold text-brand-gold">{displayName.slice(0, 1).toUpperCase()}</span><span className="hidden text-sm font-medium md:block">{displayName}</span></button>
