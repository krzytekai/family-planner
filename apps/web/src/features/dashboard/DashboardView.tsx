...
export function DashboardView({ family, displayName, todayTasks, stats, loading, error, actionError, updatingIds, canCreateTasks, onQuickAdd, onViewTasks, onViewCalendar, onViewShopping, onToggle, onDelete, unreadNotifications, onOpenNotifications, canBudget, onViewBudget }: DashboardViewProps) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] p-4 md:p-7">
      <section className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="view-heading-identity"><p className="mb-1 text-xs uppercase tracking-[.18em] text-brand-gold">{family.familyName}</p><h1 className="text-2xl font-semibold tracking-tight md:text-3xl">Dzień dobry, {displayName}! 👋</h1></div><p className="mt-1 text-sm text-brand-muted">Twoje rodzinne centrum organizacji • rola: {family.role}</p></div><QuickTaskAdd canCreate={canCreateTasks} onOpen={onQuickAdd} /></section>
