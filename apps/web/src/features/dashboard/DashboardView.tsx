...
export function DashboardView({ family, displayName, todayTasks, stats, loading, error, actionError, updatingIds, canCreateTasks, onQuickAdd, onViewTasks, onViewCalendar, onViewShopping, onToggle, onDelete, unreadNotifications, onOpenNotifications }: DashboardViewProps) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] p-4 md:p-7">
      ...
      <section className="grid grid-cols-2 gap-3 xl:grid-cols-4"><StatCard icon={CheckSquare} label="Zadania" value={taskValue} detail={taskDetail} onClick={onViewTasks}/><StatCard icon={ShoppingCart} label="Zakupy" value={shopping.loading ? '…' : shopping.error ? '—' : String(shopping.count)} detail={shopping.error ? 'Błąd danych' : 'produktów do kupienia'} onClick={onViewShopping}/><StatCard icon={WalletCards} label="Wydatki" value="—" detail="w przygotowaniu"/><StatCard icon={Bell} label="Powiadomienia" value={String(unreadNotifications)} detail="nieprzeczytanych" onClick={onOpenNotifications}/></section>
