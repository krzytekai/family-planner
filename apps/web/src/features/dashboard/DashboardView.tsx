...
export function DashboardView({ family, displayName, todayTasks, stats, loading, error, actionError, updatingIds, canCreateTasks, onQuickAdd, onViewTasks, onToggle }: DashboardViewProps) {
  ...
  return (
    <div className="mx-auto max-w-[1500px] p-4 md:p-7">
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1.05fr_1.15fr_.8fr]">
        <TodayTasksCard tasks={todayTasks} currentUserId={family.userId} currentUserRole={family.role} loading={loading} error={error} actionError={actionError} updatingIds={updatingIds} onToggle={onToggle} onViewAll={onViewTasks} />
