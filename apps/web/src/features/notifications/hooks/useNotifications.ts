...
export function useNotifications(familyId: string) {
  ...
  return { notifications: inAppNotifications, unreadCount: unreadNotificationCount(inAppNotifications), preferences, loading, saving, error, setRead, markReadById, markAllRead, savePreferences, refresh }
}
