...
interface ReminderRow { id: string; family_id: string; source_type: Reminder['sourceType']; source_id: string; title: string | null; remind_at: string; timezone: string | null; status: Reminder['status'] }
function client() { const value = getSupabaseClient(); if (!value) throw new Error('Brak konfiguracji Supabase.'); return value }
function map(row: ReminderRow): Reminder { return { id: row.id, familyId: row.family_id, sourceType: row.source_type, sourceId: row.source_id, title: row.title, remindAt: row.remind_at, timezone: row.timezone, status: row.status } }

export function createReminderRepository() {
  return {
    async list(familyId: string) {
      const { data, error } = await client().from('reminders').select('id,family_id,source_type,source_id,title,remind_at,timezone,status').eq('family_id', familyId).eq('status', 'pending').order('remind_at')
