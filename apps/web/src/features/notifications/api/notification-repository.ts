...
interface NotificationRow { id: string; family_id: string; recipient_user_id: string; notification_type: AppNotification['type']; title: string; body: string | null; source_type: AppNotification['sourceType']; source_id: string | null; read_at: string | null; dismissed_at: string | null; created_at: string }
...
function mapNotification(row: NotificationRow): AppNotification { return { id: row.id, familyId: row.family_id, recipientUserId: row.recipient_user_id, type: row.notification_type, title: row.title, body: row.body, sourceType: row.source_type, sourceId: row.source_id, readAt: row.read_at, dismissedAt: row.dismissed_at, createdAt: row.created_at } }
...
export function createNotificationRepository() {
  return {
    async list(familyId: string) {
      const { data, error } = await client().from('notifications').select('id,family_id,recipient_user_id,notification_type,title,body,source_type,source_id,read_at,dismissed_at,created_at').eq('family_id', familyId).is('dismissed_at', null).order('created_at', { ascending: false }).limit(100)
      if (error) throw new Error(error.message)
      return ((data ?? []) as NotificationRow[]).map(mapNotification)
    },
    async setRead(familyId: string, id: string, read: boolean) {
      const { error } = await client().rpc(read ? 'mark_notification_read' : 'mark_notification_unread', { target_family_id: familyId, target_notification_id: id })
      if (error) throw new Error(error.message)
    },
    async markAllRead(familyId: string) {
      const { error } = await client().rpc('mark_all_notifications_read', { target_family_id: familyId })
      if (error) throw new Error(error.message)
    },
    async dismiss(familyId: string, id: string) {
      const { error } = await client().rpc('dismiss_notification', { target_family_id: familyId, target_notification_id: id })
      if (error) throw new Error(error.message)
    },
    async dismissRead(familyId: string) {
      const { error } = await client().rpc('dismiss_read_notifications', { target_family_id: familyId })
      if (error) throw new Error(error.message)
    },
