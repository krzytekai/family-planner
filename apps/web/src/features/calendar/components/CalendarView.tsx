...
export function CalendarView({ family, createRequest, reminders, onViewTask, onReminder }: Props) {
  ...
  return <div className="mx-auto max-w-[1500px] p-4 md:p-7">
    <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="view-heading-identity"><p className="mb-1 text-xs uppercase tracking-[.18em] text-brand-gold">{family.familyName}</p><h1 className="text-2xl font-semibold tracking-tight md:text-3xl">Kalendarz rodzinny</h1></div><p className="mt-1 text-sm text-brand-muted">Wydarzenia i terminy zadań w jednym widoku.</p></div>{canCreate ? <button type="button" onClick={() => setEditor({ event: null, date: selectedDate })} className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-gold px-4 py-2.5 text-sm font-semibold text-black"><CalendarPlus className="h-4 w-4"/>Dodaj wydarzenie</button> : null}</section>
