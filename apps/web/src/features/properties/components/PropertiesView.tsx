...
import{chargeForMonth,chargesForMonth,chargesForProperty,definitionsForProperty,effectiveAmount,filterMonthlyCharges,formatPropertyMoney,isOverdue,monthKey,propertySummary,shiftMonth}from'../property-utils'
...
export function PropertiesView({family}:{family:FamilyContext}){
  ...
  const yearCharges=chargesForProperty(data.activeCharges,selectedProperty)
  const yearDefinitions=definitionsForProperty(data.definitions,data.activeCharges,selectedProperty)
  const yearSummary=propertySummary(yearCharges)
  ...
  const selectedCharge=selectedChargeId?data.charges.find(charge=>charge.id===selectedChargeId)??null:null
  ...
  const chargeCard=(charge:PropertyCharge)=>{
    ...
    return <article key={charge.id} role="button" tabIndex={0} onClick={()=>setSelectedChargeId(charge.id)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();setSelectedChargeId(charge.id)}}} className="surface cursor-pointer rounded-xl px-3 py-2.5 outline-none focus-visible:ring-2 focus-visible:ring-brand-gold sm:flex sm:items-center sm:gap-4 sm:px-4">
      ...
      <div className="mt-2 flex items-center justify-between gap-3 sm:mt-0">
        ...
        {charge.status==='pending'?<div className="flex items-center gap-3 text-[11px]"><button onClick={event=>{event.stopPropagation();setPaying(charge)}} className="min-h-10 text-brand-green">Oznacz jako zapłacone</button><button onClick={event=>{event.stopPropagation();void data.cancel(charge.id)}} className="min-h-10 text-brand-muted">Anuluj</button></div>:charge.paidAt?<p className="text-[9px] text-brand-muted">{new Date(charge.paidAt).toLocaleDateString('pl-PL')}</p>:null}
      </div>
    </article>
  }
  ...
  return <div className="mx-auto max-w-[1500px] p-4 md:p-7">
    ...
    {selectedCharge?<ChargeDetailsModal familyId={family.familyId} charge={selectedCharge} definitionName={definitionMap.get(selectedCharge.definitionId)?.name??'Opłata'} propertyName={propertyNames.get(selectedCharge.propertyId)??'Grupa opłat'} saving={data.saving} onSave={data.updateCharge} onClose={()=>setSelectedChargeId(null)}/>:null}
