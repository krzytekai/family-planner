...
describe('administration UI contract',()=>{
  ...
  it('keeps Auth Admin backend-only and restricts admin-created roles before account creation',()=>{const post=api.indexOf("request.method === 'POST'");const restriction=api.indexOf("auth.actorRole === 'admin' && role === 'admin'");const create=api.indexOf('auth.admin.auth.admin.createUser',post);expect(post).toBeGreaterThan(-1);expect(restriction).toBeGreaterThan(post);expect(create).toBeGreaterThan(restriction);expect(api).toContain('if (createdAuthUser) await cleanupCreatedAuthUser');expect(app).not.toContain('service_role')})
