...
describe('administration UI contract',()=>{
  ...
  it('does not expose member management to an adult-only family creator',()=>{expect(admin).toContain("canManageFamily=family.role==='owner'||family.role==='admin'");expect(admin).toContain('if(!canManageFamily)return')})
