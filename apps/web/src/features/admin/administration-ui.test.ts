...
describe('administration UI contract',()=>{
  ...
  it('loads members before showing owner and count',()=>{expect(admin).toContain('setLoading(true)');expect(admin).toContain('Ładowanie danych członków…');expect(admin).toContain('finally{setLoading(false)}')})
