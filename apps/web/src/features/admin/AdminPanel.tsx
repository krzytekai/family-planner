    ...
    <div className="fixed inset-0 z-[70] bg-black/70 p-3 backdrop-blur-sm sm:p-6">
      <section className="surface mx-auto flex h-full max-w-5xl flex-col overflow-hidden rounded-3xl">
        ...
        <div className="grid flex-1 gap-5 overflow-auto p-5 lg:grid-cols-[1fr_.85fr]">
          ...
          <form onSubmit={createMember} className="rounded-2xl border border-brand-gold/10 bg-brand-gold/[.03] p-4"><div className="mb-4 flex items-center gap-2 font-semibold"><Plus className="h-4 w-4 text-brand-gold"/>Dodaj użytkownika</div><div className="space-y-3">{[['Nazwa',displayName,setDisplayName,'text'],['E-mail',email,setEmail,'email'],['Hasło tymczasowe',password,setPassword,'password']].map(([label,value,setter,type])=><label key={label as string} className="block text-xs text-brand-muted">{label}<input required={label!=='Nazwa'?true:true} minLength={label==='Hasło tymczasowe'?8:undefined} type={type as string} value={value as string} onChange={(e)=>(setter as (v:string)=>void)(e.target.value)} className="mt-1 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-brand-text outline-none focus:border-brand-gold/40"/></label>)}<label className="block text-xs text-brand-muted">Rola<select value={role} onChange={(e)=>setRole(e.target.value as FamilyRole)} className="mt-1 w-full rounded-xl border border-white/10 bg-[#101017] px-3 py-2.5 text-sm text-brand-text">{roles.map((r)=><option key={r} value={r}>{r}</option>)}</select></label>{message?<p className="text-xs text-brand-muted">{message}</p>:null}<button disabled={busy} className="w-full rounded-xl bg-brand-gold px-4 py-2.5 text-sm font-semibold text-black disabled:opacity-60">{busy?'Dodawanie…':'Dodaj użytkownika'}</button></div></form>
        </div>
      </section>
    </div>
  )
