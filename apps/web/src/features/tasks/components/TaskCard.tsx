import { BellRing, CalendarClock, Check, Pencil, Repeat2, RotateCcw, Trash2, UserRound } from 'lucide-react'
...
import { canDeleteTask, canManageTaskAutomation, canUpdateTask, formatTaskDateTime, recurrenceLabel } from '../task-utils'
...
export function TaskCard({ task, currentUserId, currentUserRole, updating, onToggle, onDelete, reminder, onReminder, onEdit, onStopRecurrence }: TaskCardProps) {
  ...
  return (
    <article className="mobile-card mobile-task-card rounded-2xl border border-white/[.07] bg-black/20 p-4 transition hover:border-brand-gold/15">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-center gap-1"><span className={`rounded-full px-2 py-1 text-[10px] font-medium ${done ? 'bg-brand-green/10 text-brand-green' : task.status === 'in_progress' ? 'bg-blue-400/10 text-blue-300' : 'bg-brand-gold/10 text-brand-gold'}`}>{statusLabels[task.status]}</span>{task.recurrence?.enabled&&canManageSeries?<button type="button" onClick={()=>onStopRecurrence(task)} title="Zakończ serię" aria-label="Zakończ serię" className="grid h-11 w-11 shrink-0 place-items-center rounded-lg text-brand-muted transition hover:bg-red-400/10 hover:text-red-300"><Repeat2 className="h-3.5 w-3.5"/></button>:null}{canDelete ? <button type="button" onClick={() => onDelete(task)} aria-label={`Usuń zadanie: ${task.title}`} title="Usuń zadanie" className="grid h-11 w-11 place-items-center rounded-lg text-brand-muted transition hover:bg-red-400/10 hover:text-red-300"><Trash2 className="h-[17px] w-[17px]" /></button> : null}</div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-[11px]">
        ...
        {task.recurrence?.enabled ? <span className="inline-flex items-center gap-1 rounded-full bg-brand-gold/10 px-2 py-1 text-brand-gold"><Repeat2 className="h-3 w-3"/>Cykliczne · {recurrenceLabel(task.recurrence.rule)}</span> : null}
      </div>
      ...
      <div className="mt-4 grid grid-cols-[minmax(0,1fr)_2.75rem] items-center gap-2 sm:flex sm:flex-nowrap"><button type="button" disabled={!canEditVisibleReminder} onClick={() => assigneeReminder ? onEdit(task) : onReminder(task)} title={reminder ? formatNotificationDate(reminder.remindAt) : undefined} aria-label={reminder?`${canEditVisibleReminder ? 'Edytuj' : 'Aktywne'} przypomnienie: ${formatNotificationDate(reminder.remindAt)}`:'Ustaw przypomnienie'} className={`inline-flex min-h-11 min-w-0 w-full items-center justify-center gap-2 rounded-xl border px-2 text-xs disabled:cursor-default sm:flex-1 sm:px-3 ${reminder?'border-brand-green/20 text-brand-green hover:bg-brand-green/10 disabled:hover:bg-transparent':'border-white/10 text-brand-muted hover:border-brand-gold/20 hover:text-brand-gold'}`}><BellRing className="h-3.5 w-3.5 shrink-0"/><span className="whitespace-nowrap">{reminder ? formatNotificationDate(reminder.remindAt) : 'Przypomnij'}</span></button>{canUpdateTask(task,currentUserId,currentUserRole)?<button type="button" onClick={()=>onEdit(task)} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-white/10 text-brand-muted" aria-label={`Edytuj zadanie: ${task.title}`}><Pencil className="h-3.5 w-3.5"/></button>:null}<button
        type="button"
        disabled={!canToggle || updating}
        onClick={() => onToggle(task)}
        title={canToggle ? undefined : 'Nie masz uprawnień do zmiany tego zadania'}
        className={`col-span-2 inline-flex min-h-11 w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl border px-2 text-xs font-medium transition disabled:cursor-not-allowed disabled:opacity-45 sm:w-auto sm:flex-1 sm:px-3 ${done ? 'border-white/10 text-brand-muted hover:bg-white/5' : 'border-brand-gold/20 text-brand-gold hover:bg-brand-gold/10'}`}
      >
